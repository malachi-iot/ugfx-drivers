...
#define MAL_TEMP_SIZE 10

static union
{
    uint32_t    buf32[MAL_TEMP_SIZE];
    char        buf[sizeof(MAL_TEMP_SIZE) * 4];
}   mal_temp;
...
{
    // +++ MB
    // this *does* make a hi2u, but following acm_write doesn't produce any output
    //cdcdf_acm_write((uint8_t *)mal_temp.buf, strlen(mal_temp.buf));
    // ---
...
{
	if (state.rs232.DTR) {
        strcpy(mal_temp.buf, "hi2u");
        mal_temp.buf[4] = '0' + counter++;
        mal_temp.buf[5] = 0;
